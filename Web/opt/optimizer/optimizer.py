...
########################
class Optimize(threading.Thread):
	...
	def run(self):
		self.optimize(self.prid)	

	def optimize(self, projectid):
		def value_to_key(val, dict):
			for key in dict:
				if val == dict[key]:
					return key
		...
		lst = []
		for c1, c2 in lst:
			course1 = value_to_key(c1, dict_courses)
			course2 = value_to_key(c2, dict_courses)
			for t1, t2 in cn_times:
				
				problem += var_continues[course1][course2] == 0
		...
		# in day courses shit fuck! :))
		lst = Data.list_course_not_in_day_db(projectid)
		for c1, c2 in lst:
			course1 = value_to_key(c1, dict_courses)
			course2 = value_to_key(c2, dict_courses)
			print course1, course2, dict_courses[course1], dict_courses[course2] 
			for t1, t2 in in_day_time_list:
				const = []
						
				const.append(var_cr_time[course1][t1])
				const.append(var_cr_time[course2][t2])
				problem += lpSum(const) <= 1
			for t1, t2 in cont_times:
				const = []
